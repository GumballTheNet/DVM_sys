...
void DvmhLoop::addConsistent(DvmhData *data, DvmhAxisAlignRule axisRules[]) {
    ...
    consistents.push_back(std::make_pair(data, rules));
}

void DvmhLoop::addRemoteAccess(DvmhData *data, DvmhAxisAlignRule axisRules[]) {
    DvmhAxisAlignRule *rules = new DvmhAxisAlignRule[data->getRank()];
    typedMemcpy(rules, axisRules, data->getRank());
...
bool DvmhLoop::fillLoopDataRelations(const LoopBounds curLoopBounds[], DvmhData *data, bool forwardDirection[], Interval roundedPart[],
    ...
    for (int j = 0; j < dspace->getRank(); j++) {
        ...
        const DvmhAxisAlignRule *dataRule = data->getAlignRule()->getAxisRule(j + 1);
        if (dataRule->axisNumber >= 1 && dataRule->multiplier != 0) {
            dataAxis = dataRule->axisNumber;
            dataReverse = dataRule->multiplier < 0;
        }

        int loopAxis = -1;
        bool loopReverse = false;
        const DvmhAxisAlignRule *loopRule = alignRule->getAxisRule(j + 1);
        if (loopRule->axisNumber >= 1 && loopRule->multiplier != 0) {
            loopAxis = loopRule->axisNumber;
            loopReverse = loopRule->multiplier < 0;
        }

        if (dataAxis > 0 && loopAxis > 0) {
            forwardDirection[dataAxis - 1] = true ^ dataReverse ^ loopReverse ^ (curLoopBounds[loopAxis - 1][2] < 0);
            ...
            if ((forwardDirection[dataAxis - 1] && firstIterations) || (!forwardDirection[dataAxis - 1] && lastIterations)) {
                leftmostPart[dataAxis - 1] = true;
                roundedPart[dataAxis - 1][0] = data->getAxisLocalPart(dataAxis)[0];
            } else {
                leftmostPart[dataAxis - 1] = roundedPart[dataAxis - 1][0] == data->getAxisLocalPart(dataAxis)[0];
            }
            if ((forwardDirection[dataAxis - 1] && lastIterations) || (!forwardDirection[dataAxis - 1] && firstIterations)) {
                rightmostPart[dataAxis - 1] = true;
                roundedPart[dataAxis - 1][1] = data->getAxisLocalPart(dataAxis)[1];
            } else {
                rightmostPart[dataAxis - 1] = roundedPart[dataAxis - 1][1] == data->getAxisLocalPart(dataAxis)[1];
            }
        }
    }
...
DvmhLoop::~DvmhLoop() {
    ...
    for (int i = 0; i < (int)consistents.size(); i++) {
        assert(consistents[i].second != 0);
        delete[] consistents[i].second;
    }
    consistents.clear();
    for (int i = 0; i < (int)rmas.size(); i++) {
        assert(rmas[i].second != 0);
        delete[] rmas[i].second;
    }
    rmas.clear();
